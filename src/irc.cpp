...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GolfCoinTEST2\r");
            Send(hSocket, "WHO #GolfCoinTEST2\r");
        } else {
            // randomly join #GolfCoin00-#GolfCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #GolfCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GolfCoin%02d\r", channel_number).c_str());
        }
